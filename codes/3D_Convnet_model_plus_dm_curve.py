...
from my_classes import DataGenerator, DataGenerator_two_inputs, DataGenerator_two_inputs_test
import glob
import pandas as pd
...
def create_3D_conv_net(input_data):
    model = Conv3D(32, kernel_size=(3, 3, 3), padding='same', activation='relu', input_shape=input_shape)(input_data)
    
    model = MaxPooling3D(pool_size=(2, 2, 2))(model)

    model = Dropout(0.25)(model)
    model = Conv3D(32, kernel_size=(3, 3, 3), padding='same', activation='relu')(model)
    model = MaxPooling3D(pool_size=(2, 2, 2))(model)
    model = Conv3D(64, kernel_size=(3, 3, 3), padding='same', activation='relu')(model)
    model = MaxPooling3D(pool_size=(2, 2, 2))(model)
    model = Dropout(0.25)(model)
    model = Flatten()(model)
    model = Dense(48)(model)
...
cnn_3D_input = Input(shape=(64,64,64,1))
dm_curve_input = Input(shape=(60,))
cnn_model_3D = create_3D_conv_net(cnn_3D_input)
dm_curve_model = create_perceptron_layer(dm_curve_input)

conv = concatenate([cnn_model_3D, dm_curve_model])
...
opt = optimizers.Adam()
model = Model(inputs=[cnn_3D_input, dm_curve_input], outputs=[output])

model.compile(loss='binary_crossentropy',
              optimizer=opt,
              metrics=['accuracy'])
#model = Model(inputs=[3d_cnn_model, dm_curve_input], outputs=[output])
...
#partition['']
#
#training_generator = DataGenerator_two_inputs(partition['train'], labels_train_and_validation, **params)
#validation_generator = DataGenerator_two_inputs(partition['validation'], labels_train_and_validation, **params)
test_generator = DataGenerator_two_inputs_test(partition['test'], **params)
###print(type(partition['train']), type(partition['validation']))
#es = EarlyStopping(monitor='val_loss', mode='min', verbose=1, patience=200)
#mc = ModelCheckpoint('best_model_3d_convnet_plus_dm_curve.h5', monitor='val_accuracy', mode='max', verbose=1, save_best_only=True)
#print(model.summary())
##model.fit_generator(generator=training_generator, validation_data=validation_generator, epochs = 10, use_multiprocessing=True,workers=4)
model.load_weights('3D_Convnet_plus_dm_curve_final.h5')
predict = model.predict_generator(generator=test_generator, use_multiprocessing=False,workers=1)
np.save('predictions_htru_3D_Convnet_plus_dm_curve.npy', predict)
classified_results = np.rint(predict)
classified_results = np.argmax(classified_results, axis=1)
...
f_score = f1_score(labels_test, classified_results, average='binary')
precision = precision_score(labels_test, classified_results, average='binary')
recall = recall_score(labels_test, classified_results, average='binary')
accuracy = (labels_test == classified_results).sum()/len(labels_test)
tn, fp, fn, tp = confusion_matrix(labels_test, classified_results).ravel()
specificity = tn/(tn + fp)
gmean = math.sqrt(specificity * recall)
fpr = fp/(tn + fp)
print('HTRU-S Results from 3D Datacube plus DM-Curve:')
...
##
##
##model.save_weights('3D_Convnet_plus_dm_curve_final.h5')
#model.load_weights('3D_Convnet_plus_dm_curve_final.h5')
