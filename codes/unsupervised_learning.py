...
combined_time_freq_phase_pulsars_full_data = np.concatenate((time_phase_pulsars_combined_array, time_phase_pulsars_combined_array), axis=1)
combined_time_freq_phase_nonpulsars_full_data = np.concatenate((time_phase_nonpulsars_combined_array, time_phase_nonpulsars_combined_array), axis=1)
#padding 316 zeros so that we can reshape to 68 x 68
npad = ((0, 0), (0, 16))
combined_time_freq_phase_pulsars_full_data = np.pad(combined_time_freq_phase_pulsars_full_data, pad_width=npad, mode='constant', constant_values=0.0)
combined_time_freq_phase_nonpulsars_full_data = np.pad(combined_time_freq_phase_nonpulsars_full_data, pad_width=npad, mode='constant', constant_values=0.0)
# Reshape Arrays
...
reshaped_time_freq_phase_pulsars = [np.reshape(f,(68,68,1)) for f in combined_time_freq_phase_pulsars_full_data] 
...
time_phase_files_combined = np.concatenate((time_phase_pulsars_files, time_phase_nonpulsars_files), axis = 0)
...
time_freq_phase_data_combined = np.concatenate((reshaped_time_freq_phase_pulsars, reshaped_time_freq_phase_nonpulsars), axis = 0)
time_freq_phase_train, time_freq_phase_test, time_freq_phase_label_train, time_freq_phase_label_test = train_test_split(time_freq_phase_data_combined, time_phase_label_combined,                                          test_size=0.3, random_state=42)
...
#index_numbers_pulsars = df.loc[df['Score'] == 5].index.values.astype(int)[0:15]
#
##
#n = 15  # how many digits we will display
#plt.figure(figsize=(20, 4))
...
#    # display original
#    ax = plt.subplot(3, n, i + 1)
#    plt.imshow(time_phase_test[index_numbers_non_pulsars[i]].reshape(48, 48))
#    plt.gray()
#    ax.get_xaxis().set_visible(False)
#    ax.get_yaxis().set_visible(False)
#
#    # display reconstruction
#    ax = plt.subplot(2, n, i + 1 + n)
#    plt.imshow(time_phase_test[index_numbers_pulsars[i]].reshape(48, 48))
#    plt.gray()
#    ax.get_xaxis().set_visible(False)
#    ax.get_yaxis().set_visible(False)
#plt.savefig('results_after_cluster_classification.png')
#plt.show()
