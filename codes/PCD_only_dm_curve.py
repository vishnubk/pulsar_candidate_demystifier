...
#conv = concatenate([time_phase_model, dm_curve_model])
#conv = concatenate([time_phase_model, freq_phase_model])
conv = concatenate([time_phase_model, freq_phase_model, dm_curve_model])
#conv = pulse_profile_model

dense = Dense(128)(conv)
dense = LeakyReLU(alpha=0.1)(dense)
#dense = LeakyReLU(alpha=0.1)(conv)
...
#model = Model(inputs=[time_phase_input, freq_phase_input], outputs=[output])
model = Model(inputs=[time_phase_input, freq_phase_input, dm_curve_input], outputs=[output])
...
model.fit([time_phase_train, freq_phase_train, dm_curve_train], freq_phase_label_train , epochs=50, validation_split=0.3)



model.save_weights('pulse_profile_only.h5')
...
predict = model.predict([time_phase_test, freq_phase_test, dm_curve_test])
