...
freq_phase_train_palfa, freq_phase_test_palfa, freq_phase_label_train_palfa, freq_phase_label_test_palfa = train_test_split(freq_phase_data_combined_palfa, freq_phase_label_combined_palfa,                                          test_size=0.3, random_state=42)
...
time_phase_train_palfa, time_phase_test_palfa, time_phase_label_train_palfa, time_phase_label_test_palfa = train_test_split(time_phase_data_combined_palfa, time_phase_label_combined_palfa,                                          test_size=0.3, random_state=42)
...
pulse_profile_train_palfa, pulse_profile_test_palfa, pulse_profile_label_train_palfa, pulse_profile_label_test_palfa = train_test_split(pulse_profile_data_combined_palfa, pulse_profile_label_combined_palfa,                                          test_size=0.3, random_state=42)
...
dm_curve_train_palfa, dm_curve_test_palfa, dm_curve_label_train_palfa, dm_curve_label_test_palfa = train_test_split(dm_curve_data_combined_palfa, dm_curve_label_combined_palfa,                                          test_size=0.3, random_state=42)
...
#
#
model.fit([time_phase_train_palfa, freq_phase_train_palfa, pulse_profile_train_palfa, dm_curve_train_palfa], freq_phase_label_train_palfa , epochs=30, validation_split=0.3)
...
predict = model.predict([time_phase_test_palfa, freq_phase_test_palfa, pulse_profile_test_palfa, dm_curve_test_palfa])
...
#
#
f_score = f1_score(time_phase_label_test_palfa, classified_results, average='binary')
precision = precision_score(time_phase_label_test_palfa, classified_results, average='binary')
recall = recall_score(time_phase_label_test_palfa, classified_results, average='binary')
accuracy = (time_phase_label_test_palfa == classified_results).sum()/len(time_phase_label_test_palfa)
tn, fp, fn, tp = confusion_matrix(time_phase_label_test_palfa, classified_results).ravel() 
specificity = tn/(tn + fp)
gmean = math.sqrt(specificity * recall)
fpr = fp/(tn + fp)
print('Results with Palfa Survey:')
...
np.save('predictions_htru_from_palfa_training.npy', predict)
classified_results = np.rint(predict)
classified_results = np.reshape(classified_results, len(classified_results))
...
tn, fp, fn, tp = confusion_matrix(time_phase_label_combined, classified_results).ravel()
specificity = tn/(tn + fp)
gmean = math.sqrt(specificity * recall)
fpr = fp/(tn + fp)
print('HTRU-S Results from palfa training:')
print('Accuracy:', accuracy, 'F Score:', f_score, 'Precision:', precision, 'Recall:', recall, 'False Positive Rate:', fpr, 'Specificity:', specificity, 'G-Mean:', gmean)
